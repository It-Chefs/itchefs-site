---
title: IT Chefs Kitchen
layout: default
description: Reflex playbooks, briefs, and automation exports cooked by Chef Avian & Sous-Chef Skunkah.
header_classes: ag-ambient
---

# 👨‍🍳🍳 Welcome to the IT Chefs Kitchen

Where **Automation Meets Art™** 🛠️✨  
This is the Chef’s counter — a living fusion of ReflexRun AI recipes, automation dishes, and chef-tested briefs, plated fresh by the kitchen crew.

---

## 🛠️✨ Where Automation Meets Art™

Every reflex is a recipe.  
Every workflow is a dish.  
Every build is plated with intention and style.

---

## 🧠🍲 Core Dishes on the Menu

- 🧾 **Kitchen Briefs**  
  Your intel snapshots, simmered down into digestible weekly bites.  
  👉 [Open Kitchen Briefs]({{ site.baseurl }}/briefs/)

- 🛠️🍜 **Automation Playbooks**  
  Prepped workflows and scripted techniques — the ramen of automation.  
  👉 [Open Automation Playbooks]({{ site.baseurl }}/playbooks/)

- 🧩🥡 **Schemas & Pipelines**  
  Data ingredients boxed and ready — your mise en place for every build.  
  👉 [Open Schemas & Pipelines]({{ site.baseurl }}/schemas/)

- 🔬🍢 **Tests & Validation**  
  Taste-testing menus to ensure every reflex is perfectly seasoned.  
  👉 [Open Tests & Validation]({{ site.baseurl }}/tests/)


---

## 🎨🍰 Branding & Design Docs (Dessert Cart)

- 🍯 **Brand Matrix 2025**  
  Flavor map for your brands across platforms and products.  
  👉 [View Brand Matrix]({{ site.baseurl }}/branding/BrandMatrix_2025.md)

- ✨ **Agogy Gold Visual Template Spec**  
  House style guide for your Agogy Gold plating.  
  👉 [View Visual Template Spec]({{ site.baseurl }}/branding/Agogy_Gold_Visual_Template_Spec.md)

- 🛡️📜 **IP & Licensing Policy**  
  Legal glaze to keep your recipes protected.  
  👉 [View IP & Licensing Policy]({{ site.baseurl }}/branding/IP_and_Licensing_Policy.md)


---

## 🥇🔱 Agogy Gold Edition

This site is part of the **Agogy Gold™ Reflex Stack** —  
a curated ecosystem for builders, chefs, and architects who cook with code.  

> “Reflexes are the movements that make automation art.” — Chef Avian

---

**📦 Repo:** [itchefs-site](https://github.com/It-Chefs/itchefs-site)  
**📡 Kitchen Sync Logs:** [chefs-reflex-kitchen → Site Exports](https://github.com/It-Chefs/itchefs-site/actions)
